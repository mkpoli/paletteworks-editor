<script lang="ts">
  ...
  $: scrollModes = SCROLL_MODES.map((mode) => [
    mode,
    $LL.editor.scrollmode[mode](),
...
<div class="panel-hider" class:hidden>
  <div>
    ...
    >
      <Icon
        icon={hidden ? 'codicon:triangle-left' : 'codicon:triangle-right'}
      />
    </button>
  </div>
</div>
<div class="panel-container" class:hidden>
  <div class="panel">
    ...
    <div style="display: flex; gap: 0.5em;">
      ...
      >
        <Icon icon="fontisto:hashtag" slot="head" style="flex-shrink: 0;" />
        <span style="white-space: nowrap;" slot="tail"
          >{$LL.editor.panel.measure()}</span
        >
        <Button
          slot="action"
          class="action"
    ...
    <label for="scroll">
      ...
      <Select bind:value={scrollMode} options={scrollModes} name="scroll" />
    </label>
  </div>
  <div class="panel">
  ...
  <div class="panel">
    <Tabs>
        ...
        <ul class="visibility">
          {#each visibilitys as name}
            <li class:visible={$visibility[name]}>
              ...
              />
              <span class="title">{$LL.editor.panel.visibility[name]()}</span
              ><span class="value">{statistics[name]}</span>
            </li>
          {/each}
          <li class="other">
      ...
      <TabContent class="history-tab">
        <div class="actions">
          <Tooltip placement="bottom" keys={KEYBOARD_SHORTCUTS.undo}>
            <Button
              icon="ic:round-undo"
              on:click={() => dispatch('undo')}
              class="history-button">{$LL.editor.panel.undo()}</Button
            >
          </Tooltip>
          <Tooltip placement="bottom" keys={KEYBOARD_SHORTCUTS.redo}>
            <Button
              icon="ic:round-redo"
              on:click={() => dispatch('redo')}
              class="history-button">{$LL.editor.panel.redo()}</Button
            >
          </Tooltip>
        </div>
  ...
  <div class="panel">
    ...
    <label for="music">
      {$LL.editor.panel.musicfile()}
      {music ? filesize(music.size) : ''}
      <FileInput
        bind:file={music}
        on:open={() => {
    ...
    <label for="offset">
      ...
      <TextInput bind:value={metadata.offset} type="number" name="offset">
    ...
    <label>
      <span
        >{$LL.editor.panel.sfxvolume()}<input
          type="checkbox"
