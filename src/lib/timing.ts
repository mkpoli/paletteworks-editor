  ...
  return bpms
    ...
    .reduce(
      (acc, [tick, bpm], ind, arr) =>
        acc +
        (ind < arr.length - 1
          ? tick2secs(arr[ind + 1][0] - tick, tpb, +bpm)
          : tick2secs(targetTick - tick, tpb, +bpm)),
      0
    )
}
...
export class TimeSignatureManager {
  ...
  private find(tick: number): TimeSignatureInfo | undefined {
    return [...this.timeSignatureInfos]
      .reverse()
      .find(({ startTick }) => startTick <= tick)
  }
  ...
  tick2measure(tick: number): number {
    const { measure, startTick, beatsPerMeasure } =
      this.find(tick) ?? this.timeSignatureInfos[0]
    return Math.floor(
      measure + (tick - startTick) / TICK_PER_BEAT / beatsPerMeasure
    )
  }

  has(tick: number): boolean {
    return this.timeSignatureInfos.some(
      ({ measure }) => this.tick2measure(tick) === measure
    )
  }
