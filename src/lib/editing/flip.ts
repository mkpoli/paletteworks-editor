...
export function flipNotes(
  ...
  const flipTargets = new Map(
    notes.map((note) => [note, pickLaneFlick(flippedNote(note))])
  )
  const flipOrigins = new Map(notes.map((note) => [note, pickLaneFlick(note)]))
  return new BatchUpdate(singles, slides, flipTargets, flipOrigins, 'flip')
}

export function vflipNotes(
  singles: Single[],
  slides: Slide[],
  notes: Note[]
): BatchUpdate {
  const [minTick, maxTick] = notes.reduce(
    ([min, max], note) => {
  ...
  const flipTargets = new Map(
    notes.map((note) => [note, { tick: maxTick - note.tick + minTick }])
  )
  const flipOrigins = new Map(notes.map((note) => [note, { tick: note.tick }]))
  return new BatchUpdate(singles, slides, flipTargets, flipOrigins, 'flip')
}
