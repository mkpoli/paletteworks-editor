...
export const SCROLL_MODES = ['page', 'smooth', 'none'] as const
...
export const SCROLL_FUNCTIONS: Record<ScrollMode, ScrollFunction | undefined> =
  {
    page: (currentTick: number) => {
      ...
      return snap(
        currentTick + pos.calcDistanceTicks(MARGIN_BOTTOM),
        pos.calcDistanceTicks(pos.containerHeight) * 0.76
      )
    },
    smooth: (currentTick: number) => {
      ...
      return (
        currentTick -
        pos.calcDistanceTicks(pos.containerHeight * 0.5) +
        pos.calcDistanceTicks(MARGIN_BOTTOM)
      )
